...
gds_hash_map_keyin_fast_t * gds_hash_map_keyin_fast_new(unsigned long size,
	void *hash_cb, void *getkey_cb, void *cmpkey_cb, void *free_cb)
{
	...
	h = gds_malloc(sizeof(gds_hash_map_keyin_fast_t));
	h->map = gds_calloc(size, sizeof(gds_rbtree_keyin_fast_node_t *));
...
int gds_hash_map_keyin_fast_set_free_callback(gds_hash_map_keyin_fast_t *h,
	void *free_cb)
{
	gds_assert(h != NULL, -1);
...
void * gds_hash_map_keyin_fast_get_free_callback(gds_hash_map_keyin_fast_t *h)
{
...
unsigned long gds_hash_map_keyin_fast_hash(gds_hash_map_keyin_fast_t *h, const void *key)
{
...
void * gds_hash_map_keyin_fast_pop(gds_hash_map_keyin_fast_t *h, const void *key)
{
	unsigned long hash;
	void *data;
	...
	hash = gds_hash_map_keyin_fast_hash(h, key);
	data = gds_rbtree_keyin_fast_pop(&(h->map[hash]), key, h->getkey_cb,
		h->cmpkey_cb);
...
gds_iterator_t * gds_hash_map_keyin_fast_iterator_new(gds_hash_map_keyin_fast_t *h)
{
	...
	data = gds_malloc(sizeof(gds_hash_map_keyin_fast_iterator_data_t));

	data->hash = h;
	data->rbtree_keyin_fast_it = NULL;

	it = gds_iterator_new(data, gds_hash_map_keyin_fast_iterator_reset,
		gds_hash_map_keyin_fast_iterator_step,
		gds_hash_map_keyin_fast_iterator_get,
		gds_hash_map_keyin_fast_iterator_getkey,
		gds_hash_map_keyin_fast_iterator_data_free);
	...
	unsigned long size)
{
	...
	map = gds_calloc(size, sizeof(gds_rbtree_keyin_fast_node_t *));
	it = gds_hash_map_keyin_fast_iterator_new(h);
...
int gds_hash_map_keyin_fast_change_size(gds_hash_map_keyin_fast_t *h, unsigned long new_size)
{
	gds_rbtree_keyin_fast_node_t **map;
...
void gds_hash_map_keyin_fast_destroy(gds_hash_map_keyin_fast_t *h)
{
	if (h != NULL) {
