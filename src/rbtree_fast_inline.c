...
_Bool gds_rbtree_fast_inline_node_is_red(gds_rbtree_fast_inline_node_t *n)
{
...
gds_rbtree_fast_inline_node_t * gds_rbtree_fast_inline_node_grandparent(
	gds_rbtree_fast_inline_node_t *n)
{
	gds_rbtree_fast_inline_node_t *g = NULL;
...
gds_rbtree_fast_inline_node_t * gds_rbtree_fast_inline_node_uncle(
	gds_rbtree_fast_inline_node_t *n)
{
	gds_rbtree_fast_inline_node_t *g, *u = NULL;

	g = gds_rbtree_fast_inline_node_grandparent(n);
	if(g != NULL) {
		if(g->left == n->parent)
			u = g->right;
		else
...
gds_rbtree_fast_inline_node_t * gds_rbtree_fast_inline_node_sibling(
	gds_rbtree_fast_inline_node_t *n)
{
	gds_rbtree_fast_inline_node_t *s = NULL;
...
gds_rbtree_fast_inline_node_t * gds_rbtree_fast_inline_rotate_left(
	gds_rbtree_fast_inline_node_t *n)
{
...
gds_rbtree_fast_inline_node_t * gds_rbtree_fast_inline_rotate_right(
	gds_rbtree_fast_inline_node_t *n)
{
...
void gds_rbtree_fast_inline_node_init(gds_rbtree_fast_inline_node_t *node)
{
	node->red = true;
...
void gds_rbtree_fast_inline_rebalance_after_insert(
	gds_rbtree_fast_inline_node_t **root,
	gds_rbtree_fast_inline_node_t *node)
{
	gds_rbtree_fast_inline_node_t *u, *g;
	_Bool valid = 0;
	...
	while (!valid) {
		if (node->parent == NULL) {
			...
			*root = node;
			...
			u = gds_rbtree_fast_inline_node_uncle(node);
			if ((u != NULL) && (u->red == true)) {
				/* Parent and uncle are both red */
				node->parent->red = false;
				u->red = false;
				g = gds_rbtree_fast_inline_node_grandparent(node);
				g->red = true;
				node = g;
			} else {
				break;
	...
	if (!valid) {
		...
		&& (node->parent == g->left)) {
			gds_rbtree_fast_inline_rotate_left(node->parent);
			node = node->left;
		} else if ((node == node->parent->left)
		&& (node->parent == g->right)) {
			gds_rbtree_fast_inline_rotate_right(node->parent);
			node = node->right;
		}
		...
		if (node == node->parent->left) {
			node = gds_rbtree_fast_inline_rotate_right(g);
		} else {
			node = gds_rbtree_fast_inline_rotate_left(g);
		}
		if (node->parent == NULL)
			*root = node;
	}
...
gds_rbtree_fast_inline_node_t * gds_rbtree_fast_inline_get_node(
	...
	gds_rbtf_cmp_key_cb rbtf_cmp_key_cb, void *rbtf_cmp_key_data)
{
	gds_rbtree_fast_inline_node_t *node = root;
	int32_t cmp;
...
void gds_rbtree_fast_inline_rebalance_after_delete(
	gds_rbtree_fast_inline_node_t **root,
	gds_rbtree_fast_inline_node_t *parent,
	gds_rbtree_fast_inline_node_t *child)
{
	gds_rbtree_fast_inline_node_t *sibling, *tmp;
	bool valid = false;
	...
	while (!valid) {
		...
		if (gds_rbtree_fast_inline_node_is_red(sibling)) {
			/* Case 2: sibling is red */
			parent->red = true;
			sibling->red = false;
			if (child == parent->left) {
				tmp = gds_rbtree_fast_inline_rotate_left(parent);
				sibling = parent->right;
			} else {
				tmp = gds_rbtree_fast_inline_rotate_right(parent);
				sibling = parent->left;
			}
			if(tmp->parent == NULL)
				*root = tmp;
		...
		&& (!gds_rbtree_fast_inline_node_is_red(sibling->left))
		&& (!gds_rbtree_fast_inline_node_is_red(sibling->right)) ) {
			/* Case 3:
	...
	&& (!gds_rbtree_fast_inline_node_is_red(sibling->right)) ) {
		...
		&& (!gds_rbtree_fast_inline_node_is_red(sibling->right)) ) {
			...
			sibling = gds_rbtree_fast_inline_rotate_right(sibling);
		...
		&& (!gds_rbtree_fast_inline_node_is_red(sibling->left)) ) {
			...
			sibling = gds_rbtree_fast_inline_rotate_left(sibling);
		}
...
void gds_rbtree_fast_inline_replace_with_child(
	gds_rbtree_fast_inline_node_t **root,
	gds_rbtree_fast_inline_node_t *node,
	gds_rbtree_fast_inline_node_t *child)
{
...
int8_t gds_rbtree_fast_inline_del(gds_rbtree_fast_inline_node_t **root,
...
{
	gds_rbtree_fast_inline_node_t *node, *child, *node_to_delete;

	GDS_CHECK_ARG_NOT_NULL(root);
	GDS_CHECK_ARG_NOT_NULL(rbtf_cmp_key_cb);
	GDS_CHECK_ARG_NOT_NULL(rbtf_replace_cb);

	node = gds_rbtree_fast_inline_get_node(*root, key, rbtf_cmp_key_cb,
		rbtf_cmp_key_data);
	if(node == NULL) {
	...
	gds_rbtree_fast_inline_replace_with_child(root, node_to_delete, child);
	...
	if (node_to_delete->red == false) {
		if (child != NULL && child->red == true) {
			...
			gds_rbtree_fast_inline_rebalance_after_delete(root,
