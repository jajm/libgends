...
void t_hash_map_keyin_fast_change_size(void)
{
	gds_hash_map_keyin_fast_t *hash;
	test_t data[] = {
		{.key = "one", .value = 1},
	...
	gds_hash_map_keyin_fast_change_size(hash, 2);
	is(gds_hash_map_keyin_fast_get(hash, data[0].key), &data[0]);
	is(gds_hash_map_keyin_fast_get(hash, data[1].key), &data[1]);
	is(gds_hash_map_keyin_fast_get(hash, data[2].key), &data[2]);

	gds_hash_map_keyin_fast_change_size(hash, 64);
	is(gds_hash_map_keyin_fast_get(hash, data[0].key), &data[0]);
	is(gds_hash_map_keyin_fast_get(hash, data[1].key), &data[1]);
	is(gds_hash_map_keyin_fast_get(hash, data[2].key), &data[2]);
...
int main()
{
	...
	t_hash_map_keyin_fast_change_size();
